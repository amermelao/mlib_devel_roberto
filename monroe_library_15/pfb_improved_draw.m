...
if( cheap_sync == 1)
    ...
    sSyncOut0 = xDelay(sCoeff0SyncOut, endDelay + n_taps + 2, 'sync_delay_end0');
    sSyncOut1 = xDelay(sGenSync, endDelay + n_taps + 2, 'sync_delay_end1');
    oSyncOut.bind(sSyncOut0);
    oSyncOut1.bind(sSyncOut1);
else
    %this line for an "honest" sync delay.  More hardware expensive
    bSyncDelay = xBlock('monroe_library/sync_delay_fast', struct('delay_len', endDelay + (n_taps +2) + ((2^vector_len)*n_taps)) , {sCoeff0SyncOut}, {oSyncOut});
    bSyncDelay = xBlock('monroe_library/sync_delay_fast', struct('delay_len', endDelay + (n_taps +2) + ((2^vector_len)*n_taps)) , {sGenSync}, {oSyncOut1});
end
